...
def create_app(test_config=None):
  ...
  '''
    ...
    def get_questions():
        ...
        if len(paginated_questions) == None:
            abort(404)
  ...
  '''
    ...
    def search_questions():
        ...
        if search_term:
            search_outcome = Question.query.filter(
                Question.question.ilike(f'%{search_term}%')).all()

            return jsonify({
                'success': True,
                'questions': [question.format() for question in search_outcome],
                'total_questions': len(search_outcome),
                'current_category': None
            })
        abort(404)
  ...
  '''
    ...
    def get_quiz_question():
        body = request.get_json()
        former_Question = body.get('previous_questions')
        category = body.get('quiz_category', None)
        if former_Question is None or category is None:
            abort(422)
        if category['id'] == 0:
            questions = Question.query.filter(
                Question.id.notin_(former_Question)).all()
        else:
            questions = Question.query.filter(
